...
class CrossEntropyLoss(Module):
    ...
    def backward(self, _: None = None) -> np.ndarray:
        ...
        assert (
            len(grad.shape) == 2
        ), "CrossEntropyLoss.backward should return (batch_size, grad_size) but is {}.".format(
            grad.shape
        )
        return grad
...
class MSELoss(Module):
    ...
    def forward(self, preds: np.ndarray, targets: np.ndarray) -> float:
        ...
        Args:
            preds: Model predictions with shape (batch_size)
            targets: Ground truth targets with shape (batch_size)

        Returns:
            mean squared error loss.
        """
        assert len(preds.shape) == 2, (
            "Predictions should be of shape (batchsize, 1) "
            f"but are of shape {preds.shape}"
        )
        assert len(targets.shape) == 2, (
            "targets should be of shape (batchsize, 1) "
            f"but are of shape {targets.shape}"
        )
        assert preds.shape == targets.shape, (
            "Predictions and targets should be of same shape but are "
            f"of shapes {preds.shape} and {targets.shape}"
        )
        self.input_cache = preds, targets

        # compute the loss over the batch assuming mean reduction
        return np.sum((preds - targets) ** 2) / (2 * len(preds))
    ...
    def backward(self, _: None = None) -> np.ndarray:
        ...
        assert (
            len(grad.shape) == 2
        ), "MSELoss.backward should return (batch_size, 1)" " but is {}.".format(
            grad.shape
        )
        return grad
